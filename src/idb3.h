...
// hex printer
template<typename T>
std::string str_hex_data(const T& value)
{
...
public:
    ...
    void dump()
    {
        ...
        for (unsigned int i=0 ; i<_index.size() ; i++)
            printf("%s = %s\n",
                str_hex_data(getkey(i)).c_str(),
                str_hex_data(getval(i)).c_str());
    }
    ...
    void readkeys()
    {
        ...
        for (auto & ent : _index) {
            ...
            dbgprint("key i=%d, l=%d -> %s\n", ent.indent, klen, str_hex_data(key).c_str());
            _keys.push_back(key);
        }
    }
...
inline std::unique_ptr<BtreeBase> MakeBTree(stream_ptr  is)
{
    ...
    dbgprint("mkbt: %s\n", str_hex_data(std::string(data, data+64)).c_str());
